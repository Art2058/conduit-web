...
import { Loading, ErrorMessage, ExceptionErrorMessage } from '../ui/index.js'
...
const setData = (data, context, type) => {
	switch (type) {
		case 'local':
			...
			break
		case 'web':
			localStorage.set('cond-' + context, data)
	}
}
...
const getData = (context, storeType) => {
	switch (storeType) {
		...
		case 'web':
			return localStorage.get('cond-' + context)
	}
}
...
export default class Component extends BaseComponent {
	...
	handleError({ e, errorTypes: types, showErrorMessage, errorPosition: position }) {
		if (
			types &&
			types.length &&
